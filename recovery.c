/*
 * $Id: recovery.c,v 1.6 2007/04/19 20:01:15 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
static uint16_t crc16_for_block(char *data, int len)
{
 int remain;
 ...
 {
  crc16term=crc16tab[crc16term>>8]^((crc16term<<8)|(uint16_t)(*(tmp_dptr++)));
 }
...
int create_protfile(FILE *stream, unsigned long offset, int state)
{
 ...
 for(cur_block=0; cur_block<total_blocks; cur_block++)
 {
  ...
  while(block_offset<ifile_size)
  {
   ...
   crc32term=(uint32_t)crc16_for_block(protpad_bck+1, DEFAULT_PROTPAD_SIZE);
