/*
 * $Id: arjsec_l.c,v 1.4 2007/04/19 20:01:10 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
#ifdef SHORT_SEED
static uint8_t arjsec_seeds[]={0xDD, 0x90, 0x53, 0x1D,
                               0x77, 0x59, 0xEF, 0x3D};
#else
static uint16_t arjsec_seeds[]={0xDD0D, 0x4920, 0x5503, 0x614D,
...
void arjsec_term(uint32_t *block, uint32_t *dest, int iter)
{
 ...
 uint16_t term0, term1, term2, term3;
 ...
 for(i=0; i<iter; i++)
 {
  ...
  chksum=crc32_for_char(chksum, (uint8_t)(i%256));
  term0=(uint16_t)(chksum>>0)%8;
  term1=(uint16_t)(chksum>>5)%8;
  term2=(uint16_t)(chksum>>11)%16;
  term3=(uint16_t)(chksum>>17)%16;
  #ifdef SHORT_SEED
   term2^=((uint16_t)arjsec_seeds[term0]>>4)%16;
  #else
   term2^=((uint16_t)arjsec_seeds[term0]>>8)%16;
  #endif
...
void arjsec_crcterm(uint32_t *block, unsigned char c)
{
 ...
 block[1]*=((block[0]>>16)<<16)+(uint32_t)((uint16_t)block[0]|((uint16_t)c)<<8|1);
 block[1]++;
 block[2]+=block[0];
 block[2]+=crc32_for_char(block[1], (unsigned char)block[0]);
 hi=(uint16_t)(block[2]>>16);
 lo=(uint16_t)block[2];
...
void arjsec_read(uint32_t *block, FILE *stream, unsigned long len)
{
 ...
 while(len>0)
 {
  ...
  while((--bytes_read)>=0)
  {
   ...
   lo=(uint16_t)block[1];
   hi=(uint16_t)(block[1]>>16);
   lo2=(uint16_t)term2;
   hi2=(uint16_t)(term2>>16);
   hi*=lo2;
   hi2=hi2*lo+hi;
