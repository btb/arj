/*
 * $Id: arjcrypt.c,v 1.6 2007/04/19 20:01:10 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
static char id[]="NortheastXXXXXXXX";   /* Currently unused */
 ...
 static void entry();
 static uint16_t entry_point=(uint16_t)&entry;
 static int use_32=0;                   /* Allow 32-bit instructions */
#endif

/* Local data */

static uint32_t default_key[8]={3, 10, 6, 12, 5, 9, 0, 7};
static int last_bytes=0;                /* Number of significant bytes in the
					   last block */
static uint32_t back_code[2]={0L}; /* Recently encrypted data */
static uint32_t ext_code[2]={0L};  /* The code used by gost_cipher() */
static uint32_t gost_key[8]={0L};  /* Encryption key */
static uint32_t gost64_key[16]={0L};/* ARJCRYPT v 2.0 encryption key */
...
static void gost_loop(uint32_t *src, uint32_t *dest, uint32_t *key)
{
 uint32_t mod1, mod2;
 int i;
...
{
 uint32_t FAR *tmp_sptr;           /* Pointer to source area */
 uint32_t FAR *tmp_dptr;           /* Pointer to target area */
 int remainder;                         /* Number of bytes in the last block */

 remainder=len%8;
 if(remainder==0&&last_bytes==0)
 {
  tmp_sptr=(uint32_t *)src;
  tmp_dptr=(uint32_t *)dest;
  len>>=3;
  while(len--!=0)
  {
...
static void gost_encode(unsigned char FAR *src, unsigned char FAR *dest, int len)
{
 ...
 if(remainder==0&&last_bytes==0)
 {
  tmp_sptr=(uint32_t FAR *)src;
  tmp_dptr=(uint32_t FAR *)dest;
  len>>=3;
  while(len--!=0)
  {
...
static void gost_decode(unsigned char FAR *src, unsigned char FAR *dest, int len)
{
 ...
 uint32_t FAR *tmp_dptr;           /* Pointer to target area */
 int remainder;                         /* Number of bytes in the last block */
 uint32_t d_data;                  /* Decoded data collector */
 unsigned char *bc_offset;              /* Offset within back_code */
...
static void gost_crtkey(uint32_t *seed)
{
 uint32_t tmp_key[8];
 int i;
