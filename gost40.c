/*
 * $Id: gost40.c,v 1.4 2007/04/19 20:01:15 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
static uint32_t gost40_key[8]={0L};     /* Automatically generated key */
...
static void gost40_loop(uint32_t *src, uint32_t *dest, uint32_t *key)
{
 uint32_t mod1, mod2;
 int i;
...
{
 uint32_t *tmp_sptr;                    /* Pointer to source area */
 uint32_t *tmp_dptr;                    /* Pointer to target area */
...
static void gost40_crtkey(uint32_t *seed)
{
 uint32_t tmp_key[8];
 int i;
...
void gost40_init(unsigned char modifier)
{
 ...
 uint32_t l_modifier[2];           /* Initializer for key creation loop */
 ...
 l_modifier[1]=(int32_t)(signed char)modifier;
 last_bytes=0;
 calc_gost_pattern();
