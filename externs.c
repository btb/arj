/*
 * $Id: externs.c,v 1.10 2007/04/20 04:36:46 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
int set_target_directory;               /* Set target directory for extraction
                                           (-ht) */
#if SFX_LEVEL>=ARJ
int have_path_shortcuts AUTOINIT;       /* Command line begins with a
                                           directory specifier; all filespecs
                                           between directory specifiers are
                                           taken relative to the previous
                                           specifier */
#endif
...
uint16_t bitbuf;                        /* Used directly in the decoding */
...
#endif
uint32_t volume_crc;                    /* CRC kept for multivolume files */
...
unsigned long resume_position;          /* For multi-volume files, the number
                                           of bytes to skip. */
uint32_t header_crc;                    /* CRC-32 of current archive header */
uint32_t file_crc;                      /* CRC-32 of uncompressed file */
...
uint16_t host_data;                     /* Used for chapter information */
uint16_t entry_pos;                     /* Entryname position in filename */
...
uint16_t left[2*NC-1];                  /* Huffman tree */
uint16_t right[2*NC-1];                 /* Huffman tree */
unsigned char c_len[NC];                /* Character length */
uint16_t cpos;                          /* Position in out_buffer */
...
uint16_t reg_id AUTOINIT;               /* SFX registration signature */
