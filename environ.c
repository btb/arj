/*
 * $Id: environ.c,v 1.34 2017/05/29 21:16:10 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
static int roll_dirent(struct new_ffblk *new_ffblk)
{
 ...
 {
  if(strlen(ent->d_name)+l>=CCHMAXPATH)
  {
   new_ffblk->ff_name[l]='\0';
   msg_cprintf(H_ERR, M_MAXPATH_EXCEEDED, CCHMAXPATH, new_ffblk->ff_name);
...
#define S_ISSOCK(m)  0
       ...
       (attrs&FATTR_DT_UXSPECIAL)))
   {
    ...
    new_ffblk->l_search.l_key.dev=real_dev;
    new_ffblk->l_search.l_key.inode=real_inode;
...
/* Reports if the filespecs in argtable can expand to duplicate filenames.
   0 = no duplicates expected. */

#if SFX_LEVEL>=ARJ
int preventive_dupname_check(char **argtable, int args)
{
 int cur_arg, cx_arg;
...
void get_mode_str(char *str, unsigned int mode)
{
  ...
  str[0]='-';
  str[1]=(mode&FATTR_IRUSR)?'r':'-';
  str[2]=(mode&FATTR_IWUSR)?'w':'-';
  str[3]=(mode&FATTR_SUID)?((mode&FATTR_IXUSR)?'s':'S')
                          :((mode&FATTR_IXUSR)?'x':'-');
  str[4]=(mode&FATTR_IRGRP)?'r':'-';
  str[5]=(mode&FATTR_IWGRP)?'w':'-';
  str[6]=(mode&FATTR_SGID)?((mode&FATTR_IXGRP)?'s':'S')
                          :((mode&FATTR_IXGRP)?'x':'-');
  str[7]=(mode&FATTR_IROTH)?'r':'-';
  str[8]=(mode&FATTR_IWOTH)?'w':'-';
  str[9]=(mode&FATTR_SVTX)?((mode&FATTR_IXOTH)?'t':'T')
                          :((mode&FATTR_IXOTH)?'x':'-');
  str[10]=' ';
  str[11]=' ';
  str[12]=' ';
...
static int wild_proc(char *wild, char *name, uint32_t *trail, unsigned int first, unsigned int nesting)
{
...
int match_wildcard(char *name, char *wcard)
{
  ...
  uint32_t trail[(MAXWILDAST*CCHMAXPATHCOMP+31)>>5];
  int l;
 ...
 if(recurse_subdirs)
 {
  ...
  while(rc==0)
  {
   if(
      ...
      /* Break symlink loops by keeping track of inodes. The entry in suspicion
         of creating a loop will be stored but won't be recursed into. To
         tolerate some weird systems (suppose someone tries it under mingw),
         zeros are not accepted as valid inode numbers. But if zero inodes are
         present on a system that supports symlinks, loops are still
         possible! */
      &&(pnew_ffblk->l_search.l_key.inode==0||!link_search(&sl_entries, &pnew_ffblk->l_search, NULL, 0))
#endif
     )
   {
    /* Reallocate filename buffers with a margin for subdir wildcards */
