/*
 * $Id: garble.c,v 1.8 2007/04/19 20:01:15 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
{
 return((char FAR *)((uint32_t)((FP_OFF(long_addr)>>4)+((uint32_t)FP_SEG(long_addr))<<16)+(uint32_t)(FP_SEG(long_addr)%16)));
}
#endif
...
static int arjcrypt_init(char *name)
{
 ...
 fclose(stream);
  ...
  arjcrypt_body=(char FAR *)((uint32_t)arjcrypt_psp<<16);
...
int garble_init(char modifier)
{
  ...
  else                                   /* Assume that ARJCRYPT is needed */
  {
   ...
   exblock.l_modifier[1]=(int32_t)(signed char)modifier;
