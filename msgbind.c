/*
 * $Id: msgbind.c,v 1.9 2016/05/08 17:11:43 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
/* Transform "\n", "\r", "\a" and "\b" characters into the corresponding ASCII
   equivalents */
...
void put_hdr(FILE *file, char *name, char *src)
{
 ...
 fprintf(file, "/*\n"
          " * %s, %04u/%02u/%02u\n"
          " * ---------------------------------------------------------------------------\n"
          " * Do not modify this file. It was automatically generated by MSGBIND from\n"
          " * %s.\n"
...
int main(int argc, char **argv)
{
 ...
 char *tbufptr;                         /* Working pointer into the buffer */
 ...
 printf("MSGBIND v 1.67  [08/05/2016]  ARJ helper tool\n\n");
 if(argc<6)
 {
 ...
 while(get_msg(resfile, target, c_system, binding, locale, msg_buffer, MSG_SIZE)!=NULL)
 {
  ...
  if(strcmp(msgpool, SKIP))
  {
   ...
   strcat(pool[tpool].data, ", ");
   tbufptr=msg_buffer+1;
   buf_len=strlen(tbufptr);
   tbufptr[--buf_len]='\0';
   patch_string(tbufptr);
   crc32term=pool[tpool].crc32;
   crc32_for_string(tbufptr);
