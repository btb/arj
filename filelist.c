/*
 * $Id: filelist.c,v 1.9 2007/04/19 20:01:14 andrew_belov Exp $
 * ---------------------------------------------------------------------------
 * This file implements filelists and link candidate  lists along with service
 * routines  for XMS  and caching/swapping.  It  consists  of  two  completely
 * different code groups, one for  full-featured filelists of ARJ, and another
 * one for simplified filelists (SIMPLE_FLIST) employed by ARJSFX.
 *
 * Link candidates are managed as a resident linked list with little tolerance
...
static unsigned long crc_matches, hkey_matches;
...
static void set_hkey(char FAR *hkeys, uint32_t crc)
{
 hkeys[(crc&HKEY_MASK)>>3]|=(1<<((crc&HKEY_MASK)&7));
}

/* Checks for a head key to avoid hash and filename matching */

static int check_hkey(char FAR *hkeys, uint32_t crc)
{
 return((hkeys[(crc&HKEY_MASK)>>3]&(1<<((crc&HKEY_MASK)&7)))?1:0);
}
...
#endif
{
  ...
  free(tmp_name);
  if(!check_hkey(root->table->hkeys, crc32term))
   return(0);
  hkey_matches++;
  crc_seed=(CRC_KEY)crc32term;
  ...
  for(curblock=root->table->low_block; curblock<=root->table->hiblock; curblock++)
  {
   if(curblock!=root->table->block_to_flush)
   {
    ...
    {
     if(idblock_ptr->keys[cur_entry]==crc_seed)
     {
      crc_matches++;
...
static void cache_cleanup(struct flist_root *root)
{
 if(!root->table->not_allocated)
 {
  ...
  if(root->table->hkeys!=NULL)
  {
   farfree(root->table->hkeys);
   root->table->hkeys=NULL;
  }
...
void flist_cleanup_proc(struct flist_root *root)
{
 ...
 if(debug_enabled&&strchr(debug_opt, 'v')!=NULL)
 {
  ...
  if(root==&flist_main)
   msg_cprintf(0, M_HASH_MATCHES, crc_matches, hkey_matches);
 }
 if(root->storage==BST_FAR)
 {
...
void retrieve_entry(char *dest, struct file_properties *properties, struct flist_root *root, FILE_COUNT entry)
{
 ...
 struct disk_file_info FAR *dptr;
 unsigned int idx;                      /* Temporary idblock index */
 int curblock;
...
int add_entry(struct flist_root *root, char *name, FILE_COUNT *count, struct file_properties *properties)
{
 ...
 #endif
 uint32_t tmp_crc;
 unsigned int tmp_offset;               /* Offset to fileinfo within blocks */
 ...
 tmp_offset=idblock_ptr->size;
 /* Check against capacity/size limits */
 if(idblock_ptr->total_entries+1>ENTRIES_PER_BLOCK||
    ((long)tmp_offset+(long)extend_len+1>(long)(FLIST_BLOCK_SIZE-sizeof(struct idblock)-2)))
 {
  save_heap_block(root, (char FAR *)root->table->sec_cache);
 ...
 idblock_ptr->sub_offset[index]=tmp_offset;
 ...
 if(root->type!=FL_STANDARD)
 {
  ...
  tmp_crc=crc32term;
  set_hkey(root->table->hkeys, tmp_crc);
  idblock_ptr->keys[index]=(CRC_KEY)tmp_crc;
 }
 if(count!=NULL)
  (*count)++;
 return(0);
...
#endif
{
  ...
  if(root->type!=FL_STANDARD)
  {
   root->table->hkeys=farmalloc_msg(HKEY_TABLE_SIZE);
   far_memset(root->table->hkeys, 0, HKEY_TABLE_SIZE);
  }
 #else
  root->maxfiles=maxfiles;
...
/* Record-keeping for link tracking */

#if SFX_LEVEL>=ARJ&&TARGET==UNIX

/* Initializes an entry list for the first time */

void init_l_entries(struct l_entries *entries)
{
 memset(entries, 0, sizeof(struct l_entries));
}

/* Resets a link list, reclaiming the memory */

void flush_l_entries(struct l_entries *entries)
{
 struct l_search_array FAR *cur, *next;

 for(cur=entries->first; cur!=NULL; cur=next)
 {
  next=cur->next;
  farfree(cur);
 }
 init_l_entries(entries);
}

/* [Hard]link search routine. Either returns a pointer to an existing
   entry in l_search structure, or creates a new entry. For hardlink
   resolution, the properties structure is filled with target data.
   In symlink mode the routine is fed with directory inodes to eliminate
   circular links. */
...
int link_search(struct l_entries *entries, struct l_search *l_search, struct file_properties *properties, FILE_COUNT ref)
{
 ...
 struct l_key cmp_key;

 cmp_key=l_search->l_key;
 crc32term=CRC_MASK;
 crc32_for_block((char *)&cmp_key, sizeof(&cmp_key));
 if(check_hkey(entries->hkeys, crc32term))
 {
  for(cur=entries->first; cur!=NULL; cur=cur->next)
  {
   c=(cur->next==NULL)?entries->last_filled:L_ENTRIES_INCREMENT;
   for(i=0; i<c; i++)
   {
    /* Refcounts are not compared (they may be changed by some external activity
       during the directory traversal phase). Zero/negative refcounts should be
       weeded out by caller. Zero inodes are OK here. */
    if(!far_memcmp(&l_search->l_key, &cur->s[i].l_key, sizeof(struct l_key)))
    {
     if(properties!=NULL)
     {
      properties->islink=1;
      properties->l_search.ref=cur->s[i].ref;
  ...
  set_hkey(entries->hkeys, crc32term);
 if(entries->cur==NULL||entries->last_filled==L_ENTRIES_INCREMENT)
 {
  cur=farmalloc_msg(sizeof(struct l_search_array));
  if(entries->cur==NULL)
   entries->first=entries->cur=cur;
  else
  {
   entries->cur->next=cur;
   entries->cur=cur;
  }
  entries->last_filled=0;
  cur->next=NULL;
 }
 else
  cur=entries->cur;
 cur->s[entries->last_filled]=*l_search;
 cur->s[entries->last_filled].ref=ref;
 entries->last_filled++;
 return(0);
}
