/*
 * $Id: misc.c,v 1.6 2007/03/11 17:59:12 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
int flist_add_files(struct flist_root *root, struct flist_root *search_flist, char *name, int expand_wildcards, int recurse_subdirs, int file_type, FILE_COUNT *count)
{
 ...
 char *tmp_name;
  ...
  #if TARGET==UNIX
   /* Clear the symlink tracking list to permit further searches visit the same subdirectories */
   flush_l_entries(&sl_entries);
  #endif
 #else
