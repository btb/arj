   ...
   НОВЫЕ ФУНКЦИОНАЛЬНЫЕ ВОЗМОЖНОСТИ И ОТЛИЧИЯ ОТ ИСХОДНОГО ARJ
      ...
      *  Во  избежание  двусмысленности  при указании  в первом  параметре
         командной строки базового каталога последующие файлы ищутся в нем
	 только если в их именах нет разделителей каталогов. Поиск файлов,
	 в   именах    которых    присутствуют  разделители,   выполняется
	 относительно текущего каталога.

         Если  такая форма  используется с  параметром "-e1" для архивации
	 файлов из различных подкаталогов, для которых имя общего базового
	 каталога в  архиве  должно  быть опущено, то указывать имя такого
	 каталога следует при помощи параметра "-ht".
