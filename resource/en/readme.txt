   ...
   NEW FEATURES AND DIFFERENCES FROM THE ORIGINAL ARJ
      ...
      *  For consistency, the first filename specified in the command line
         is only  treated as the  base directory for  subsequent filenames
	 that do not  contain path  separators. Any filename containing  a
	 path  separator  will  be  referenced  to the  current  directory
	 instead.
	 
	 When this  syntax is  used together  with the "-e1"  parameter to
	 archive a subtree, the base directory part to be  ommitted can be
	 specified with the "-ht" parameter.

         e.g. the command:
          arj a archive -e1 basedir/ file0 subdir1/file1 subdir2/file2
	 should be rewritten as:
	  arj a archive -e1 -htbasedir/ file0 subdir1/file1 subdir2/file2
