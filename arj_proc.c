/*
 * $Id: arj_proc.c,v 1.26 2016/05/08 20:04:59 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
int *sw_index[]={&skip_integrity_test, &prompt_for_directory, &skip_ts_check,
                 ...
                 &execute_extr_cmd, &lowercase_names, &arjdisp_enabled, NULL};
static
...
/* Appends a path separator to the given pathname if it doesn't contain one */
...
/* Deletes the ending path separator if there's one. Root path specifiers may
   be spared, or separators can be deleted unconditionally */
...
{
 unsigned long pvol, arjsec_overhead;
 long remain;
...
int check_integrity(char *name)
{
 ...
 char pattern[20];
 /* 32-bit CRC and file size as stored in the file */
 uint32_t st_crc;
 unsigned long st_fsize=0;
...
void init()
{
 ...
 av_total_longnames=0;
  ...
  #if TARGET==UNIX
   init_l_entries(&l_entries);
   init_l_entries(&sl_entries);
  #endif
 #endif
...
static int get_str_from_jq()
{
 ...
 if(*sptr!='\0')
 {
  ...
  endofs=i;
  i=0;
  while(i<endofs&&patterns<SEARCH_STR_MAX)
  {
   while(sptr[i]=='\0'&&i<endofs)
    i++;
   if(i<endofs)
   {
    search_str[patterns++]=sptr+i;
    while(sptr[i]!='\0')
     i++;
   }
  }
