/*
 * $Id: arj_arcv.c,v 1.20 2016/05/08 20:04:59 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
/* Returns CRC-32 of the given file */

static uint32_t crc32_for_file(char *name)
{
 FILE *stream;
 char *buffer;
 int block_size;
...
int pack_file(int is_update, int is_replace)
{
 ...
 else if(file_type==ARJT_BINARY||file_type==ARJT_TEXT)
 {
  ...
  if(file_type==ARJT_BINARY)
   pack_file_proc(data_pos);
  /* ASR fix 08/05/2016 - raise the flag here instead of check_multivolume()
     to avoid depending on compression routine context. This is now consistent
     with the handing in proc_ext_hdr(). */
  if(origsize<uncompsize&&multivolume_option)
   volume_flag_set=1;
 }
 if(eh!=NULL)
