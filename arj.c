/*
 * $Id: arj.c,v 1.17 2016/05/08 19:45:26 andrew_belov Exp $
 * ---------------------------------------------------------------------------
...
static void parse_arj_sw(int cmd, char *arj_sw, char *dest)
{
...
static int preprocess_cmdline(int argc, char *argv[])
{
 int i;
 int cmd=0;
 char *arg;
...
/* Parses the next command line argument, taking filename argument. Returns the command
   code. */

static int parse_cmdline_arg(char *token, int cmd)
{
 int name_len;
 int valid_file_arg;
 int end_pos;
 ...
 if(is_switch(token))                   /* Tokens may appear anywhere */
  analyze_arg(token);
 else if(cmd==0)                        /* Command goes first */
 {
  cmd=toupper(token[0]);
  /* Use fake 1-character commands for 2-character commands */
 ...
 else if(file_args<0)                   /* Filenames begin with archive name */
 {
 ...
 else                                   /* Handle all subsequent filenames */
 {
  unix_path_to_dos(token);
  name_len=strlen(token);
  end_pos=name_len-1;
  end_sym=token[end_pos];
  while(token[end_pos]==' ')
   end_pos--;
  if(file_args==0||strcmp_os(token, nonexist_name))
  {
   valid_file_arg=1;
   /* Handle possible target directory specifiers */
   if(file_args==0)
   {
    /* Addition commands must specify an existing directory as their first argument */
    if(!set_target_directory&&msg_strchr(M_ADD_COMMANDS, (char)cmd)||cmd==ARJ_CMD_ADDC)
    {
     tmp_path=malloc_msg(end_pos+1);
     memcpy(tmp_path, token, end_pos);
     tmp_path[end_pos]='\0';
     trim_pathsep(tmp_path, 1);
     have_path_shortcuts=is_directory(tmp_path);
    }
    /* Other commands must either specify a slash-terminated pathname or an existing
       directory without any intervening wildcards */
    else if(target_dir[0]=='\0')
    {  
     if(strchr(path_separators, token[end_pos])!=NULL)
     {
      target_dir=token;
      if(end_sym!=' ')
       return(cmd);
     ...
     else if(msg_strchr(M_DIR_COMMANDS, (char)cmd)!=NULL&&strpbrk(token, wildcard_pattern)==NULL&&
             is_directory(token))
     {
...
/* Parses the command line from a response file or argv[] */

static int parse_cmdline(char *rsp_name, int argc, char **argv)
{
 int cur_arg;
 int cmd=0;
 FILE *stream;
 ...
 if(rsp_name[0]=='\0')
 {
  for(cur_arg=1; cur_arg<argc; cur_arg++)
   cmd=parse_cmdline_arg(argv[cur_arg], cmd);
 }
 else
 {
  ...
  while(fgets(header, FILENAME_MAX, stream)!=NULL)
  {
   ...
   while((endptr-tptr)>0)
   {
    ...
    if((endptr-tptr)>0)
    {
     cmd=parse_cmdline_arg(tptr, cmd);
     while(*tptr!='\0'&&((endptr-tptr)>0))
      tptr++;
    }
   }
...
int main(int argc, char *argv[])
{
 ...
 int j, l;
 FILE *stream;
 char *tmp_ptr, *tptr, *shortcut_stem=NULL;
 char *name;
 int flist_type;
 FILE_COUNT numfiles;
 ...
 cmd=parse_cmdline(rsp_name, argc, argv);
 if(install_errhdl)
  ignore_errors=1;
 if(force_lfn)
  lfn_supported=LFN_SUPPORTED;
 ...
 {
  tptr=f_arg_array[j];
  if(!have_path_shortcuts&&strcmp_os(tptr, nonexist_name)&&is_directory(tptr))
  {
 ...
 if(cmd!=ARJ_CMD_ORDER)
  flist_type=preventive_dupname_check(f_arg_array, file_args)?FL_HASH:FL_STANDARD;
 else
 ...
 /* Process all filename arguments from the command line or file list */
 for(j=limit; j<file_args; j++)
 {
  tptr=f_arg_array[j];
  if(listchars_allowed&&tptr[0]==listchar) /* Handle list files */
  {
   if(*++tptr=='\0')
   ...
   while(fgets(tmp_ptr, FILENAME_MAX, stream)!=NULL)
   {
    ...
    if(tmp_ptr[0]!='\0')
    {
     if(is_add_cmd&&set_target_directory)
      strcpy(name, target_dir);
     else if(shortcut_stem!=NULL)
      strcpy(name, shortcut_stem);
     else
      name[0]='\0';
     strcat(name, tmp_ptr);
     count=0;
  ...
  /* Isolate directory shortcuts when using addition commands. Shortcuts are
     identified by trailing path separators. The last (or the only) filespec
     in the command-line cannot serve as a shortcut. */
  else if(is_add_cmd&&have_path_shortcuts&&
          j<file_args-1&&(l=strlen(f_arg_array[j]))>0&&strchr(path_separators, f_arg_array[j][l-1])!=NULL)
  {
   if(shortcut_stem!=NULL)
   {
    free(shortcut_stem);
    shortcut_stem=NULL;
   }
   shortcut_stem=malloc_msg(strlen(f_arg_array[j])+1);
   strcpy(shortcut_stem, f_arg_array[j]);
   order[j]=1;
  }
  else                                  /* All other filespecs are to be
                                           handled normally */
  {
   /* Reset any effective path shortcuts if path separators are present */
   if(have_path_shortcuts&&strpbrk(f_arg_array[j], path_separators)!=NULL&&shortcut_stem!=NULL)
   {
    free(shortcut_stem);
    shortcut_stem=NULL;
   }
   ...
   /* BUGBUG: f_arg_array[j]=strdup(name); is needed for M_CANTFIND to be
      less puzzling. Consider reimplementing the array to allow for dynamic
      allocations. */
   count=0;
   if(flist_add_files(&flist_main, &flist_exclusion, name, expand_wildcards, recurse_subdirs, allow_any_attrs, &count))
   {
